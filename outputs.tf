output "user_name" {
  ...
  value       = concat(aws_iam_user.default.*.name, list(""))[0]
}

output "user_arn" {
  description = "The ARN assigned by AWS for this user"
  value       = concat(aws_iam_user.default.*.arn, list(""))[0]
}

output "user_unique_id" {
  description = "The unique ID assigned by AWS"
  value       = concat(aws_iam_user.default.*.unique_id, list(""))[0]
}

output "user_login_profile_key_fingerprint" {
  description = "The fingerprint of the PGP key used to encrypt the password"
  value       = concat(aws_iam_user_login_profile.default.*.key_fingerprint, list(""))[0]
}

output "user_login_profile_encrypted_password" {
  description = "The encrypted password, base64 encoded"
  value       = concat(aws_iam_user_login_profile.default.*.encrypted_password, list(""))[0]
}
